...
const SignUp = async (request, response) => {
  try {
    let { username, email, password, cpassword, phone, location } =
      request.body;
    let role = "user";
    ...
    if (oldUser) {
      response
        ...
        .json("Email is Already Exists (<:>)");
    } else if (password != cpassword) {
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Password Not Equal Cpassword (<:>)");
    } else {
    ...
    response
      ...
      .json("Error In Sign Up Function (<:>)");
  }
};
...
const SignIn = async (request, response) => {
  try {
    ...
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Sign In Function (<:>)");
  }
};
  ...
  try {
    ...
    if (oldUser) {
      ...
      response
        ...
        .json("Profile Updated Successfully (<:>)");
    } else
      response.status(StatusCodes.BAD_REQUEST).json("Account Not Found (<:>)");
  } catch (error) {
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Update Profile Function (<:>)");
  }
};
...
const Update_Password = async (request, response) => {
  try {
    ...
    if (oldUser) {
      ...
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Account Is Not Found (<:>)");
    }
  } catch (error) {
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Update Password Function (<:>)");
  }
};
...
const Deactivate = async (request, response) => {
  try {
    ...
    const oldUser = await users.updateOne(
      { _id: userId, isBlocked: false, isDeleted: false, isActivated: true },
      { isActivated: false }
    );
    if (oldUser.modifiedCount) {
      response
        .status(StatusCodes.OK)
        .json("Account Deactivate Successfully (<:>)");
    } else
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Account Is Not Found Or Deactivated Already (<:>)");
  } catch (error) {
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Deactivate Function (<:>)");
  }
};
...
const Activate = async (request, response) => {
  try {
    const userId = request.decoded.id;
    const oldUser = await users.updateOne(
      { _id: userId, isBlocked: false, isDeleted: false, isActivated: false },
      { isActivated: true }
    );
    if (oldUser.modifiedCount) {
      response
        .status(StatusCodes.OK)
        .json("Account Activate Successfully (<:>)");
    } else
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Account Is Not Found Or Activated Already (<:>)");
  } catch (error) {
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Activate Function (<:>)");
  }
};
...
const Get_All_Users = async (request, response) => {
  try {
    ...
    if (oldUser) {
      ...
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Your Account Not Found (<:>)");
  } catch (error) {
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Get All Users Function (<:>)");
  }
};
  ...
  try {
    ...
    if (oldUser) {
      ...
      if (data.modifiedCount)
        ...
        response
          .status(StatusCodes.BAD_REQUEST)
          .json("Account Is Not Found Or Blocked Or Alread Admin (<:>)");
    } else
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Your Account Not Found (<:>)");
...
const Get_All_Admins = async (request, response) => {
  try {
    ...
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Get All Admins Function (<:>)");
  }
};
  ...
  try {
    ...
    if (oldUser) {
      ...
      if (data.modifiedCount)
        response
          .status(StatusCodes.OK)
          .json("Admin Deleted Successfully (<:>)");
      else
        response
          .status(StatusCodes.BAD_REQUEST)
          .json("Account Is Not Admin (<:>)");
    } else
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Your Account Not Found (<:>)");
  ...
  try {
    ...
    if (oldUser) {
      ...
      if (data.modifiedCount)
        response
          .status(StatusCodes.OK)
          .json("Account Blocked Successfully (<:>)");
      else
        response
          .status(StatusCodes.BAD_REQUEST)
          .json("Your Account Not Found Or Already Blocked (<:>)");
    } else
      response
        .status(StatusCodes.BAD_REQUEST)
        .json("Your Account Not Found (<:>)");
    ...
    response
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json("Error In Block User Function (<:>)");
  }
};
